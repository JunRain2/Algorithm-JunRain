...
public class 팰린드롬_페어 {
	static class TrieNode {
		...
		List<Integer> palindromicWordIds;

		public TrieNode() {
			// 단어 ID의 쵯값은 의미 없는 값으로 선언
			this.wordId = -1;
			// 자식 노드는 알파벳의 개수인 최대 26개까지 가능
			this.children = new TrieNode[26];
			// 팰린드롬 단어 ID 목록을 빈 값으로 초기화
			this.palindromicWordIds = new ArrayList<>();
		}
	}
	...
	static class Trie{
		...
		public boolean isPalindrome(String str, int start, int end) {
			...
			// 한 번도 틀리지 않을 경우 팰린드롬이므로 true 반환
			return true;
		}
		...
		public void insert(int index, String word) {
			...
			for (int i = word.length() - 1; i >= 0; i--) {
				...
				if (isPalindrome(word, 0, i)) {
					cur.palindromicWordIds.add(index);
				}
		...
		public List<List<Integer>> search(int index, String word) {
			...
			for(int palindromicWordId : cur.palindromicWordIds) {
				result.add(Arrays.asList(new Integer[] {index, palindromicWordId}));
			}
			// 결과 리턴
			return result;
		}
