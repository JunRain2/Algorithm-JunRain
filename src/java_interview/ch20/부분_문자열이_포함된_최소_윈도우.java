package java_interview.ch20;

import java.util.HashMap;
import java.util.Map;

public class 부분_문자열이_포함된_최소_윈도우 {
	public String minWindow(String s, String t) {
		// 필요한 문자의 카운터를 정의하는 맵
		Map<Character, Integer> need = new HashMap<>();
		// 찾아야 하는 문자열의 모든 문자 삽입, 찾아야 하는 문자는 0 이상의 값이 되며, 나머지 문자는 모두 0 이하의 값이다.
		...
		for (char c : s.toCharArray()) {
			...
			if (missing == 0) {
				// need의 값이 0 미만인 경우에는 증가시키며 왼쪽 포인터 계속 이동
